...
public:
    ...
    static void printWeights(const Matrix<float> &weights) {
        ...
        for (size_t i = 0; i < weights.getNumCols(); i++) {
            for (size_t j = 0; j < weights.getNumRows(); j++) {
                std::cout << i << " -(" << std::fixed << std::setprecision(DECIMAL_PLACES_IN_PRINT)
                          << weights.getItem(j, i) << ")→ " << j << "    ";
            }
            std::cout << std::endl;
        }
    }
     ...
     */
    static void
    printWeightStats(const Matrix<float> &weights, bool amountWeights = false, bool printWeightMatrix = false) {
